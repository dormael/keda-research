...
KEDA의 **두뇌** 역할을 하는 메인 컨트롤러입니다. Kubernetes의 Controller 패턴을 따르며, ScaledObject/ScaledJob CRD가 생성·수정·삭제될 때마다 Reconcile 루프를 실행합니다.
...
**상세 동작:**
- ScaledObject의 `Reconcile()` 루프에서 대상 Deployment/StatefulSet을 확인하고, 트리거에 맞는 **스케일러 인스턴스를 생성**
- `ScaleHandler`가 각 스케일러를 통해 외부 시스템에 직접 폴링하여 메트릭을 수집
- 수집된 메트릭에 따라 **0↔1 전환은 직접 Kubernetes Scale API를 호출**하여 처리
- 1→N 범위의 스케일링은 자동으로 **HPA를 생성·관리**하여 Kubernetes 네이티브 로직에 위임
- ScaledJob의 경우 이벤트마다 독립 Job을 생성하는 별도 Reconcile 루프 운영

**핵심 소스 코드:**
| 로직 | 소스 코드 | 설명 |
|------|-----------|------|
| Reconcile 루프 | [`scaledobject_controller.go#L118-L179`](https://github.com/kedacore/keda/blob/main/controllers/keda/scaledobject_controller.go#L118-L179) | ScaledObject 변경 감지 → 스케일러 연결 → HPA 관리 진입점 |
| HPA 생성 | [`hpa.go#L96-L126`](https://github.com/kedacore/keda/blob/main/controllers/keda/hpa.go#L96-L126) | `newHPAForScaledObject()` — External Metrics 기반 HPA 스펙 조립 |
| HPA 업데이트/삭제 | [`hpa.go#L167-L197`](https://github.com/kedacore/keda/blob/main/controllers/keda/hpa.go#L167-L197) | `updateHPAIfNeeded()` → `deleteHPA()` — HPA 수명주기 관리 |
| 폴링 루프 | [`scale_handler.go#L193-L220`](https://github.com/kedacore/keda/blob/main/pkg/scaling/scale_handler.go#L193-L220) | `startScaleLoop()` — pollingInterval마다 `checkScalers()` 반복 실행 |
| 스케일링 판단 | [`scale_handler.go#L222-L259`](https://github.com/kedacore/keda/blob/main/pkg/scaling/scale_handler.go#L222-L259) | `checkScalers()` — 스케일러 상태 평가 후 `RequestScale` 호출 |
| Active 여부 판단 | [`scale_handler.go#L571-L677`](https://github.com/kedacore/keda/blob/main/pkg/scaling/scale_handler.go#L571-L677) | `getScaledObjectState()` — 전체 스케일러 메트릭 종합 → `isActive` 반환 |
| **0→1 활성화** | [`scale_scaledobjects.go#L371-L381`](https://github.com/kedacore/keda/blob/main/pkg/scaling/executor/scale_scaledobjects.go#L371-L381) | `scaleFromZeroOrIdle()` — replicas를 minReplicaCount 또는 1로 설정 |
| **→0 비활성화** | [`scale_scaledobjects.go#L310-L329`](https://github.com/kedacore/keda/blob/main/pkg/scaling/executor/scale_scaledobjects.go#L310-L329) | `scaleToZeroOrIdle()` — cooldown 확인 후 replicas를 0으로 축소 |
| cooldown 체크 | [`scale_scaledobjects.go#L291-L296`](https://github.com/kedacore/keda/blob/main/pkg/scaling/executor/scale_scaledobjects.go#L291-L296) | cooldownPeriod / initialCooldownPeriod 경과 여부 확인 |
| K8s Scale API 호출 | [`scale_scaledobjects.go#L420-L428`](https://github.com/kedacore/keda/blob/main/pkg/scaling/executor/scale_scaledobjects.go#L420-L428) | `scaleClient.Scales().Update()` — 실제 레플리카 수 변경 |

#### 2) `keda-operator-metrics-apiserver` (Metrics Server)

Kubernetes의 **External Metrics API**를 구현하는 어댑터 서버입니다. HPA가 스케일링 결정을 내리려면 표준화된 메트릭 API가 필요한데, 이 서버가 KEDA 스케일러의 원시 데이터를 HPA가 이해할 수 있는 형태로 변환합니다.
...
**상세 동작:**
- Kubernetes API Aggregation Layer에 **External Metrics Provider**로 등록됨
- HPA가 `external.metrics.k8s.io` API를 통해 메트릭을 요청하면, 이 서버가 keda-operator의 메트릭 서비스(gRPC)에서 데이터를 가져와 응답
- 스케일러가 수집한 원시 값(예: Kafka 컨슈머 랙 = 150)을 `ExternalMetricValue` 형태로 정규화
- HPA는 이 정규화된 값과 ScaledObject의 `threshold`를 비교하여 필요한 레플리카 수를 계산

**핵심 소스 코드:**
| 로직 | 소스 코드 | 설명 |
|------|-----------|------|
| gRPC 서버 정의 | [`server.go#L36-L44`](https://github.com/kedacore/keda/blob/main/pkg/metricsservice/server.go#L36-L44) | `GrpcServer` 구조체 — ScaleHandler 참조를 보유 |
| 메트릭 서빙 | [`server.go#L47-L62`](https://github.com/kedacore/keda/blob/main/pkg/metricsservice/server.go#L47-L62) | `GetMetrics()` gRPC 핸들러 — ScaleHandler에서 메트릭을 가져와 HPA에 전달 |
| 메트릭 수집 위임 | [`scale_handler.go#L367-L534`](https://github.com/kedacore/keda/blob/main/pkg/scaling/scale_handler.go#L367-L534) | `GetScaledObjectMetrics()` — 모든 스케일러에서 병렬로 메트릭 수집 |

#### 3) `keda-admission-webhooks`

Kubernetes **ValidatingWebhookConfiguration**을 활용하여 KEDA CRD의 생성·변경 요청을 사전 검증하는 방어 계층입니다.
...
**상세 동작:**
- Kubernetes API Server가 ScaledObject/ScaledJob/TriggerAuthentication CRD의 CREATE·UPDATE 요청을 수신하면, 이 webhook에 검증 요청을 전달
- **충돌 검증**: 동일 Deployment를 타겟팅하는 중복 ScaledObject 차단
- **값 범위 검증**: `minReplicaCount` ≤ `maxReplicaCount`, `pollingInterval` > 0 등 논리적 일관성 확인
- **트리거 검증**: 스케일러 타입명의 유효성, 필수 파라미터 존재 여부 확인
- 검증 실패 시 Kubernetes API가 요청을 거부하고 사용자에게 명확한 오류 메시지 반환

**핵심 소스 코드:**
| 로직 | 소스 코드 | 설명 |
|------|-----------|------|
| CRD 타입 정의 | [`apis/keda/v1alpha1/`](https://github.com/kedacore/keda/tree/main/apis/keda/v1alpha1) | ScaledObject, ScaledJob, TriggerAuthentication 스펙 및 검증 로직 |

#### 4) 공통 핵심: 스케일러 시스템 (Scaler System)

세 컴포넌트를 관통하는 핵심 추상화는 **Scaler 인터페이스**입니다. 모든 70개+ 스케일러가 이 인터페이스를 구현하며, operator가 폴링 시 이를 호출합니다.

**핵심 소스 코드:**
| 로직 | 소스 코드 | 설명 |
|------|-----------|------|
| Scaler 인터페이스 | [`scaler.go#L44-L57`](https://github.com/kedacore/keda/blob/main/pkg/scalers/scaler.go#L44-L57) | `GetMetricsAndActivity()`, `GetMetricSpecForScaling()`, `Close()` 정의 |
| PushScaler 인터페이스 | [`scaler.go#L59-L63`](https://github.com/kedacore/keda/blob/main/pkg/scalers/scaler.go#L59-L63) | Push 기반 스케일러용 확장 인터페이스 — `Run()` 메서드 추가 |
| 스케일러 팩토리 | [`scalers_builder.go#L101-L187`](https://github.com/kedacore/keda/blob/main/pkg/scaling/scalers_builder.go#L101-L187) | `buildScaler()` — 트리거 타입명 → 스케일러 생성자 매핑 (80개+ switch case) |
| 개별 스케일러 상태 평가 | [`scale_handler.go#L679-L776`](https://github.com/kedacore/keda/blob/main/pkg/scaling/scale_handler.go#L679-L776) | `getScalerState()` — 개별 스케일러의 활성화 여부 판단 + fallback 처리 |
