...
```
[외부 이벤트 소스] ←── 직접 폴링 ──→ [keda-operator]
       (Kafka, SQS, RabbitMQ 등)              │
                                               │ 이벤트 감지 시 0→1 활성화
                                               ▼
...
| # | 제목 | 비고 |
|---|------|------|
| 48 | Architectural Resilience and Event-Driven Elasticity | NotebookLM 생성 분석 텍스트 (KEDA 아키텍처 종합 분석) |
...
13. **데모 / 실습**: ScaledObject 예제 (Kafka 또는 RabbitMQ)
14. **Q&A**
