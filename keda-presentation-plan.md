...
flowchart TD
    ...
    SO -- "1→N 스케일링 위임" --> MA["keda-metrics-apiserver
    (외부 메트릭 변환)"]

    MA --> HPA["HPA"]
    HPA --> Pod["Pod 수 조절"]

    style ES fill:#f5f5f5,stroke:#999
    style KO fill:#e8f0fe,stroke:#4a86c8
    style SO fill:#e8f0fe,stroke:#4a86c8
    style MA fill:#e8f0fe,stroke:#4a86c8
    style HPA fill:#f5f5f5,stroke:#999
    style Pod fill:#f5f5f5,stroke:#999
```
